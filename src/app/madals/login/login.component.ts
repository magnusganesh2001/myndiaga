...
export class LoginComponent implements OnInit {
  ...
  constructor(public modalController: ModalController,private _nav:NavService,private toastr:ToastrService,private router :Router,private _location:Location, public _communicate:CommunicateService,private _ApiServiceService:NavService,) {}
  ...
  onSubmitRegistration(data: any){
    ...
    this._ApiServiceService.signUpCustomer(opost).subscribe(
      res=>{
        ...
        if(res.length == 0)
          {
            this.toastr.errorToast('Wrong Password Or Phone Is');
            this.router.navigate([currentUrl])
          }else{
            localStorage.setItem('token',res.token.token);
            localStorage.setItem('name',res.name);
      ...
      err=>{
        this.toastr.warnToast(err.error.err_message)
    ...
    this._ApiServiceService.postPhone(login).subscribe(
      res=>{
        ...
        if(res.phone != null)
        {
          ...
          clicking?.click();
          this.toastr.warnToast('You Are Already Registered With This Number');
        }
        else{
      ...
      error =>{
        this.toastr.warnToast(error.error.errors.phone[0]);
      }
    )
  }
  ...
  resendOtp()
  {
    ...
    this._ApiServiceService.postPhone(login).subscribe(
      res=>{
        ...
        if(res.phone == null)
        {
          this.toastr.successToast('OTP send successfully');
        }
  ...
  resetPassword(value:any)
  {
    ...
    this._ApiServiceService.resetPassword(reset).subscribe(
      res=>{
          if(res==true)
          {
            this.toastr.successToast("Pasword Changed");
            this.router.navigate([currentUrl])
            .then(() => {
              window.location.reload();
            });
        ...
        this.toastr.warnToast(err.err_message);
        // this.toastr.warning();
      }
    )
